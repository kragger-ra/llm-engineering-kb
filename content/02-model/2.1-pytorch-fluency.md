...
Файнтюн (`2.2-finetuning`) и любая отладка обучения требуют понимать, что
...
- **Порядок включения рычагов при OOM:** bf16 → gradient checkpointing → gradient
  accumulation (меньший micro-batch) → LoRA/QLoRA (`2.2-finetuning`) → шардинг
...
- Дальше: `2.5-gpu-minimum` (бюджет памяти обучения, 16 байт/параметр);
  `2.2-finetuning` (SFT/LoRA-цикл поверх этих рычагов).
